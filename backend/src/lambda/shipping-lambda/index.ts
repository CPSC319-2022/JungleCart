...
exports.handler = async function (event) {
  ...
  const buyerId = JSON.parse(
    JSON.stringify(await SQLManager.query(sqlGetBuyerId))
  )[0].buyer_id;
  const sqlGetBuyerName = `SELECT first_name FROM dev.user u WHERE u.id = ${buyerId}`;
  const buyerName = JSON.parse(
    JSON.stringify(await SQLManager.query(sqlGetBuyerName))
  )[0].first_name;
  const sqlGetBuyerEmail = `SELECT email FROM dev.user u WHERE u.id = ${buyerId}`;
  const destinationEmail = JSON.parse(
    JSON.stringify(await SQLManager.query(sqlGetBuyerEmail))
  )[0].email;
  const sqlGetOrderItems = `SELECT * from dev.order_item oi WHERE oi.order_id = ${orderId}`;
  const orderItems = JSON.parse(
    JSON.stringify(await SQLManager.query(sqlGetOrderItems))
  );
  ...
  const currentOrderStatus = (await SQLManager.query(sqlGetOrderStatus))[0]
    .order_status_id;
  const ordered = 1;
  ...
  if (currentOrderStatus == ordered) {
    for (let i = 0; i < orderItems.length; i++) {
      ...
      tempQueryResult = JSON.parse(
        JSON.stringify(await SQLManager.query(sqlGetProductName))
      )[0].name;
    ...
    const deliveryRes = await ses
      .sendEmail(deliveryEmailParams)
      .promise()
      .then((result) => {

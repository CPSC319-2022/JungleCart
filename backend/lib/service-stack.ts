...
export class ServiceStack extends EnvironmentStack {
  ...
  constructor(scope: Construct, id: string, props: ServiceStackProps) {
    ...
    const layerConfigNames = props.layerConfigNames
      ? props.layerConfigNames
      : [];

    layerConfigNames.forEach((layerConfigName) => {
      const layerConfig = this.node.tryGetContext(props.environment)[
        'layer-config'
      ][layerConfigName];
      convertParamsToContext(layerConfig, this);
      this.layers[layerConfigName] = lambda.LayerVersion.fromLayerVersionArn(
        this,
        id + layerConfig.LAYER_ID,
        layerConfig.LAYER_VERSION_ARN
      );
    });
  ...
  private setLambdaEnvironment(
    environment: string,
    lambdaEnvironmentConfigNames: [key: string]
  ) {

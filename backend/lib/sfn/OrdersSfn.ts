...
export class OrdersSfn extends ServiceStepFunction {
  ...
  updateStepFunction() {
    const orderUpdate = new StepFunctionFacade().createStepFunction(this.scope, "OrderUpdateSfn",
      {
        config: this.config,
  ...
  createStateMachine() {
    const orderValidationRequest = new sfn.Pass(this.scope, `${this.id}-` + "validate-order", {
      parameters: {
        pathParameters: {
    ...
    const placeOrderReq = sfn.Condition.stringEquals('$.requestContext.httpMethod', 'POST');

    const orders = new tasks.LambdaInvoke(this.scope,`${this.id}-`+ "getOrder", {
    ...
    const orderStatusCheck = sfn.Condition.stringEquals('$.order.status_label', 'pending');


    const paymentRequest = new sfn.Pass(this.scope, `${this.id}-`+ "paymentCheck", {
      parameters: {
        "order.$": "$.order",
        pathParameters: {
          "orderId.$": "$.order.id",
    ...
    const payment = new tasks.LambdaInvoke(this.scope, `${this.id}-`+ "validatePayment", {
    ...
    const orderRequest = new sfn.Pass(this.scope, `${this.id}-`+  "orderRequest", {
      parameters: {
        "body.$": "$.body",

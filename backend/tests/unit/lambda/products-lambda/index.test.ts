...
describe('Unit tests for Products', () => {
    ...
    before(() => {
        ...
        response = new Response(() => {/* nothing */}, () => {/* nothing */});
    });
    ...
    describe('When getting Products', () => {
        ...
        it('should throw an error if product id is missing', async () => {
            const body = data;
            expect(controller.getProductById(body, response)).to.eventually.be.rejected;
        });

        it('should successfully get product info', () => {
            const body = data;
            expect(controller.getProductById(body, response)).to.eventually.be.fulfilled;
        });
    });

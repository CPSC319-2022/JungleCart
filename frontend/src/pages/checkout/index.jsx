...
const Checkout = () => {
  ...
  return (
    <main className={`${styles.container}`}>
      <div
        className={'rounded-md p-4 flex flex-col text-start  py-2   w-full '}
      >
        <h1 className={'font-bold text-2xl py-2'}>
          Order #{pendingOrder?.id} Checkout
        </h1>
        <p className={'font-medium w-md'}>
          Your order is now pending and the items on hold. Please review your
          order information. If you have made a mistake you can click on cancel
          at the bottom of the page and all your cart items will be restored.
      ...
      <div className={styles.content}>
        <ul className="steps steps-vertical w-full py-1  ">
          <li
            className={`w-full max-w-full step w-full text-start justify-start py-1 `}
          >
            <section
              className={`${styles.block} rounded-md p-4 flex flex-col text-start border  border-base-200 mt-0 shadow-sm  w-full `}
            >
              ...
              <div className={'flex flex-row gap-x-3 justify-between'}>
                ...
                {address_loading ? (
                  ...
                  Object.keys(preferredAddress).length !== 0 ? (
                  ...
                  <div className={styles.block}>
                    ...
                    <p className={styles.editPrompt}>
                      Set a preferred address in{' '}
                      <Link href="/profile">Profile</Link> to continue with
                      checkout
                    </p>
                  </div>
                )}
          ...
          >
            <section
              className={`${styles.block} rounded-md p-4 flex flex-col text-start border border-base-200 mt-0 shadow-md w-full `}
            >
              <div className={'flex flex-row gap-x-3 justify-between'}>
                <h2>Payment</h2>
                <p className={styles.editPrompt}>
              ...
              <div className={'card bg-gray-light p-2 rounded-md w-full'}>
                <div className={'flex gap-x-2 py-1 items-center '}>
                  ...
                  <p className={'text-sm'}>
                    For orders to be placed you need to have a valid payment
                    method set up in your profile.
                  </p>
                </div>
              </div>
            </section>
          ...
          >
            ...
            >
              ...
              <Separator />
              <p>
                After a successful order placement your order will be marked as
                orders and you will receive an email at{' '}
                <span className={'underline font-black text-primary-dark'}>
                  {user.email}
                </span>
              </p>

              <p>
                You can cancel your order before your order is{' '}
                <span className={'font-black '}>Shipped</span>. In that
                timeframe you can cancel your order at{' '}
                <Link className={'font-bold'} href="/orders">
                  /orders
                </Link>
              </p>
              <Separator />
              <div className={'card bg-gray-light p-2 rounded-md w-full'}>
                <div className={'flex gap-x-2 py-1 items-center '}>
                  ...
                  <p className={'text-xs '}>
                    When an order is successfully placed, per AWS free-tier
                    restrictions only the verified test accounts provided will

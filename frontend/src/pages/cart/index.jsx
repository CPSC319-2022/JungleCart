...
const Cart = () => {
  ...
  const handleChange = (id, updatedTotal) => {
    ...
    if (Math.floor(Number(updatedTotal)) !== Number(updatedTotal)) {
      showPopup(popupStates.WARNING, 'Quantity cannot have fractions');
      return;
    }
      ...
      <section>
        ...
        <div className={'card bg-gray-light p-2 rounded-md w-full'}>
          <div className={'flex gap-x-2 py-1 items-center '}>
            ...
            <p className={'text-sm'}>
              Items in your cart can be sold out or modified at any point. but
              don&apos;t worry we will prevent you from moving onto checkout if
              that case happens
            </p>
          </div>
        </div>
        ...
        <div className={'card bg-gray-light p-2 rounded-md w-full'}>
          <div className={'flex gap-x-2 py-1 items-center '}>
            ...
            <p className={'text-sm'}>
              Easily modify a product in your cart by either clicking on trash
              icon to delete, or click on the quantity bubble to edit the
              quantity. You can discard the change by clicking on the X icon or
              apply the change by clicking the check icon.{' '}
            </p>
          </div>
        </div>
          ...
          <table className="table table-compact w-full shadow-2xl border border-base-200 border-b-0">
            ...
            <tbody>
              ...
              <tr className={'border-b border-base-200 '}>
                ...
                <td
                  className={
                    'flex justify-start border-l border-base-200 text-center h-full bg-base-200 '
                  }
                >
                  <div
                    className={
                      ' pl-3  w-fit h-full font-black text-md bg-base-200 '
                    }
                  >
                    TOTAL
                  </div>
                </td>
                <td className="font-black text-md bg-base-200 rounded-sm text-primary-dark">
        ...
        <div className={'card bg-gray-light p-2 rounded-md w-full'}>
          <div className={'flex gap-x-2 py-1 items-center '}>
            ...
            <p className={'text-sm'}>
              The total does not include any shipping and tax fees that may be
        ...
        <div className={'flex gap-x-2 py-1 items-center'}>
          ...
          <p className={'text-xs '}>
            To provide even more flexibility, we have plans to incorporate bulk
            editing
          </p>
        </div>
      </section>
...
const CartItem = ({ product, handleDelete, handleChange }) => {
  ...
  return (
    <tr key={product.id} className="">
      <th className={'border-r border-b border-base-200'}>{product.id}</th>
      ...
      <td className={'py-2 '}>
        <div
          className={
            'flex gap-x-3 rounded-2xl bg-primary border border-primary p-0 w-[200px]'
          }
        >
          {isUnchaged() && (
            <div

...
const OrdersPage = () => {
  ...
  const flattenedOrders = orders.orders.reduce((orders, order) => {
    ...
    const orderItems = products.map(item => ({ ...item, date: order.created_at }))
    return [...orders, ...orderItems]
  }, [])

  const deliveredOrders = flattenedOrders.filter(order => order.shipping_status === "delivered")
  const shippingOrders = flattenedOrders.filter(order => order.shipping_status !== "delivered")
          ...
          <ShadedCard key={order.id}>
            ...
            <CardBottom className={styles.cardBottom}>
              <div className={styles.col}>
                ...
                <p>{new Intl.DateTimeFormat("en-US", options).format(new Date())}</p>
              </div>
